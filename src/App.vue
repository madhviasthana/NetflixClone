...
const currentSlide = ref(0); // Syncs with the carousel
let currentMovieIndex = 0; // Tracks the currently selected movie

onMounted(() => {
  console.log('Component mounted, setting initial movie.');

  // Set initial movie and slide
  setTimeout(() => {
...
function moveToNextMovie() {
  const currentCategory = movies[0]; // Assuming first category
  if (currentMovieIndex < currentCategory.length - 1) {
    currentMovieIndex++;
  } else {
    currentMovieIndex = 0; // Loop back to the start
  }
...
function updateMovieSelection() {
  ...
  currentSlide.value = currentMovieIndex; // Sync with the carousel
  console.log('Current movie:', movie.value, 'Current slide:', currentSlide.value);
}

// Sync the slide index with carousel events
const updateMovieIndex = (newIndex) => {
  currentMovieIndex = newIndex;
  movie.value = movies[0][currentMovieIndex]; // Update movie
};
</script>
  ...
  <div class="fixed w-full h-screen bg-black">
    ...
    <div v-if="!showFullVideo">
      ...
      <div class="fixed z-30 bottom-0 right-0 w-full h-[55%] pl-[120px] overflow-y-auto">
        <!-- Pass currentSlide and update logic to VideoCarousel -->
        <VideoCarousel 
          class="pb-14 pt-14" 
          category="Popular Movies" 
          :movies="movies[0]" 
          :currentMovieIndex="currentSlide" 
          @updateSlide="updateMovieIndex" 
        />
        <VideoCarousel 
          class="pb-14 pt-14" 
